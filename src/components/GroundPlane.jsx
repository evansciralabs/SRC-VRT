...
export default function GroundPlane({ children, isPitchMode, hardwareTrigger, groundImage, setGroundImage, isAmbi, theme, themeCfg }) {
  ...
  const tolerance = 3;
  const isGroundLevel = Math.abs(tilt.beta) < tolerance && Math.abs(tilt.gamma) < tolerance;

  // Wall mode covers both axis conventions across devices/browsers:
  //   beta-dominant: portrait hold, beta swings toward ±90°
  //   gamma-dominant: landscape hold or some Android browsers, gamma hits ±90°
  const isBetaWall  = (Math.abs(tilt.beta - 90) < tolerance || Math.abs(tilt.beta + 90) < tolerance) && Math.abs(tilt.gamma) < 15;
  const isGammaWall = (Math.abs(tilt.gamma - 90) < tolerance || Math.abs(tilt.gamma + 90) < tolerance) && Math.abs(tilt.beta) < 15;
