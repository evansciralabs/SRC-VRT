...
export default function GroundPlane({ children, isPitchMode, hardwareTrigger, groundImage, setGroundImage, isAmbi, theme, themeCfg }) {
  ...
  // DUAL-AXIS LEVEL DETECTION
  // Replaces the single `isLevel` boolean. Authorizes capture on either a
  // flat ground plane (beta ≈ 0°) or a vertical wall plane (beta ≈ ±90°).
  // Everything else in the component is unchanged from V1.
  const groundTolerance = 3;
  const wallTolerance   = 25; // wide enough to catch close-range diagonal holds
  const isGroundLevel = Math.abs(tilt.beta) < groundTolerance && Math.abs(tilt.gamma) < groundTolerance;

  // Anything "mostly upright" qualifies as wall — beta between ~65° and ~115°
  const isBetaWall  = (Math.abs(tilt.beta - 90) < wallTolerance || Math.abs(tilt.beta + 90) < wallTolerance);
  const isGammaWall = (Math.abs(tilt.gamma - 90) < wallTolerance || Math.abs(tilt.gamma + 90) < wallTolerance);
  const isWallLevel = isBetaWall || isGammaWall;

  const isLevel  = isGroundLevel || isWallLevel;
  const axisMode = isGroundLevel ? 'GROUND' : isWallLevel ? 'WALL' : 'SEEKING...';

  // Bubble re-centers around whichever axis is dominant
  const bubbleY = isBetaWall ? (tilt.beta - (tilt.beta > 0 ? 90 : -90)) * 3 : tilt.beta * 3;
  const bubbleX = isGammaWall ? (tilt.gamma - (tilt.gamma > 0 ? 90 : -90)) * 3 : tilt.gamma * 3;
  ...
  return (
    <div
      className={`ground-plane-container absolute inset-0 w-full h-full overflow-hidden flex items-center justify-center ${isPitchMode ? '' : 'transition-colors duration-300'} ${isPitchMode ? 'bg-transparent' : theme === 'daylight' ? 'bg-[#e4e4e7]' : 'bg-[#111]'}`}
      style={isPitchMode ? { backgroundColor: 'transparent' } : undefined}
    >
      
      <div className={`absolute inset-0 z-10 flex items-center justify-center ${isPitchMode ? 'overflow-visible' : 'overflow-hidden'}`}>
            ...
            {hasGyroPermission && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
                ...
                <div className="absolute top-32 text-white/70 font-mono text-xs text-center font-bold tracking-widest drop-shadow-md">
                  ...
                  <p className={isLevel ? 'text-green-400' : 'text-red-400'}>{axisMode}</p>
                </div>
              </div>
            )}
          </>
