...
import CalibrationAnchor from './components/CalibrationAnchor';
...
const extractVeilpointPayload = (rawString) => {
  ...
  let css = cssMatch
    ? cssMatch.map((m) => m.replace(/<\/?style[^>]*>/gi, '')).join('\n')
    : '';
...
const getThemeStyles = (theme) => {
  if (theme === 'daylight')
    return {
      ...
      btnDefault:
        'bg-white border border-blue-400 text-blue-500 shadow-none hover:bg-blue-50 transition-colors',
      btnDanger:
        'bg-white border border-red-400 text-red-500 shadow-none hover:bg-red-50 transition-colors',
  ...
  if (theme === 'ghost')
    return {
      ...
      btnDefault:
        'bg-transparent border border-gray-700 text-gray-600 shadow-none hover:bg-gray-900 transition-colors',
      btnDanger:
        'bg-transparent border border-red-900 text-red-900 shadow-none hover:bg-red-950 transition-colors',
  ...
  return {
    appBg: 'bg-black',
    panel:
      'bg-[#112222]/95 border border-cyan-400 text-cyan-400 shadow-[0_0_15px_rgba(0,255,204,0.5)]',
    btnDefault:
      'bg-black/80 border border-cyan-400 text-cyan-400 shadow-[0_0_10px_rgba(0,255,204,0.3)] hover:bg-cyan-900 transition-colors',
    btnDanger:
      'bg-black/80 border border-red-500 text-red-500 shadow-[0_0_10px_rgba(255,0,0,0.3)] hover:bg-red-900 transition-colors',
...
export default function App() {
  ...
  const [calibrationRatio, setCalibrationRatio] = useState(null); // physical/px
  const [calibrationUnit, setCalibrationUnit] = useState('ft');
  // payloadScale lives in ArtPlane; we mirror it here for the live readout
  const [mirroredScale, setMirroredScale] = useState(1);
  // ─────────────────────────────────────────────────────────────────────────
    ...
    for (const file of files) {
      if (file.type.startsWith('image/')) {
        ...
        try {
          ...
          const recursiveSearch = (obj, parentLabel = null) => {
            ...
            if (obj.scratchpad) {
              Object.keys(obj.scratchpad).forEach((tabKey) => {
                ...
                if (payload)
                  extractedPayloads.push({ id: parentLabel || tabKey.toUpperCase(), ...payload });
              });
            }
            Object.keys(obj).forEach((key) => {
              ...
              if (
                ...
                obj.attachments.includes(nextObj)
              ) {
          ...
          setTimeout(
            () => setPayloads((prev) => prev.filter((p) => p.id !== errPayload.id)),
            2000
          );
        }
      }
  ...
  // ── STAMP EXECUTION ───────────────────────────────────────────────────────
  // Bakes the active payload + its current matrix/scale into the static background array.
  // The ArtPlane resets to center after firing this, ready for the next node.
  const executeStampLayer = (currentMatrix, currentScale) => {
    if (payloads.length === 0) return;
    const currentPayload = payloads[activePayloadIdx];

    setStampedLayers((prev) => [
      ...prev,
      {
        id: `${currentPayload.id}-${Date.now()}`,
        css: currentPayload.css,
    ...
    // Auto-cycle to next payload if multiple are loaded
    if (payloads.length > 1) {
      setActivePayloadIdx((p) => (p < payloads.length - 1 ? p + 1 : 0));
    }
  };

  const undoLastStamp = () => {
    setStampedLayers((prev) => prev.slice(0, -1));
  };
  // ─────────────────────────────────────────────────────────────────────────

  // ── RENDER PIPELINE ───────────────────────────────────────────────────────
  // Captures: stamped background layers + active ArtPlane layer.
  // Unused payloads are never rendered into the export ref, so they drop cleanly.
  const executeRenderPipeline = async () => {
    ...
    try {
      ...
      link.download = `${exportPrefix}_${shortHash}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
  ...
  return (
    <div
      className={`w-full h-screen font-mono overflow-hidden relative transition-colors duration-300 ${themeCfg.appBg} ${themeCfg.textMain}`}
    >
      {/* VRT BRANDING (MODE TOGGLE) */}
      <div
        onClick={handleVRTTap}
        className={`absolute top-4 left-1/2 -translate-x-1/2 text-sm font-bold tracking-[0.3em] cursor-pointer select-none z-[60] transition-colors ${
          theme === 'daylight'
            ? 'text-blue-500'
            : theme === 'ghost'
            ? 'text-gray-700'
      ...
      <div
        ...
        className={`absolute top-4 ${
          isAmbi ? 'left-4' : 'right-4'
        } text-sm font-bold tracking-[0.3em] cursor-pointer select-none z-[60] transition-colors ${
          theme === 'daylight'
      ...
      {/* ── EXPORT PREFIX HUD ────────────────────────────────────────────── */}
      {/* Discrete prefix input — tap to edit, tap away or press Enter to confirm */}
      <div className="absolute top-10 left-1/2 -translate-x-1/2 z-[60] flex items-center justify-center">
        {isPrefixEditing ? (
          <input
            autoFocus
            type="text"
            ...
            onBlur={() => setIsPrefixEditing(false)}
            onKeyDown={(e) => e.key === 'Enter' && setIsPrefixEditing(false)}
            maxLength={24}
            className={`text-[9px] font-mono tracking-widest text-center bg-transparent border-b outline-none w-36 pb-0.5 ${
              theme === 'daylight'
                ? 'text-slate-400 border-slate-300'
                : theme === 'ghost'
                ? 'text-gray-600 border-gray-700'
                : 'text-cyan-600/70 border-cyan-800'
            }`}
          />
        ) : (
          <button
            onClick={() => setIsPrefixEditing(true)}
            className={`text-[9px] font-mono tracking-widest opacity-50 hover:opacity-80 transition-opacity ${
              theme === 'daylight' ? 'text-slate-400' : theme === 'ghost' ? 'text-gray-700' : 'text-cyan-700'
            }`}
            title="Edit export prefix"
          >
            {exportPrefix}
          </button>
        )}
      ...
      <div
        className={`absolute top-1/2 -translate-y-1/2 flex flex-col gap-8 z-[60] pointer-events-auto transition-all duration-300 ${
          isAmbi ? 'right-4' : 'left-4'
        }`}
      >
        <button
          onClick={() => setHardwareTrigger((prev) => prev + 1)}
          className={`w-12 h-12 rounded-full border flex items-center justify-center text-xl active:scale-90 transition-colors ${getToolStyle('cam')}`}
        >
          ᛰ
        </button>
        <label
          className={`w-12 h-12 rounded-full border flex items-center justify-center text-xl active:scale-90 cursor-pointer transition-colors ${getToolStyle('import')}`}
        >
          ⤓
          <input
            type="file"
        ...
        {/* CALIBRATION ANCHOR TOGGLE */}
        <button
          onClick={() => setIsCalibrating((c) => !c)}
          className={`w-12 h-12 rounded-full border flex items-center justify-center text-xl active:scale-90 transition-colors ${
            isCalibrating
              ? 'bg-amber-950 border-amber-500 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.4)]'
              : getToolStyle('import')
          }`}
      ...
      <main className="w-full h-full">
        <div
          ...
          className={`w-full h-full relative overflow-hidden transition-colors duration-300 ${
            theme === 'daylight' ? 'bg-[#ffffff]' : 'bg-[#0a0a0a]'
          }`}
        >
          <GroundPlane
            ...
            themeCfg={themeCfg}
          >
            {/* ── CALIBRATION ANCHOR (above ground, below stamps) ───────── */}
            <CalibrationAnchor
              isActive={isCalibrating && !isPitchMode}
              themeCfg={themeCfg}
              payloadScale={mirroredScale}
              artPlaneBaseSize={240}
              onRatioChange={setCalibrationRatio}
            />
            {/* ── STAMPED BACKGROUND LAYERS ──────────────────────────────── */}
            {/* Pointer-events none: these are baked, fully inert */}
              ...
              {stampedLayers.map((layer) => (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className="[&>svg]:max-w-full [&>svg]:max-h-full w-full h-full flex items-center justify-center"
            ...
            {/* Active ArtPlane sits on top of stamped layers (z-40) */}
            <ArtPlane
              isPitchMode={isPitchMode}
              ...
              onCyclePrev={
                payloads.length > 1
                  ? () => setActivePayloadIdx((p) => (p > 0 ? p - 1 : payloads.length - 1))
                  : undefined
              }
              onCycleNext={
                payloads.length > 1
              ...
              hasStampedLayers={stampedLayers.length > 0}
              onUndoStamp={undoLastStamp}
            >
              {hasActivePayload ? (
                <div className="w-full h-full flex items-center justify-center pointer-events-none">
                  <style
                    dangerouslySetInnerHTML={{ __html: payloads[activePayloadIdx].css }}
                  />
                  <div
                    className="[&>svg]:max-w-full [&>svg]:max-h-full w-full h-full flex items-center justify-center"
      ...
      {payloads.length > 0 && !isPitchMode && (
        <div className="absolute bottom-40 left-1/2 -translate-x-1/2 z-[70] pointer-events-none text-center w-full max-w-[280px]">
          <span
            className={`px-4 py-1 text-[10px] font-mono font-bold tracking-widest border-b rounded-full shadow-lg ${
              themeCfg.appBg === 'bg-[#f4f4f5]'
                ? 'text-slate-500 border-slate-300 bg-white/50'
                : 'text-cyan-400 border-cyan-500/30 bg-black/50 backdrop-blur-sm'
            }`}
          >
            NODE: {payloads[activePayloadIdx].id}
            {stampedLayers.length > 0 && (
              <span className="ml-2 text-violet-400">// {stampedLayers.length} STAMPED</span>
            )}
          </span>
        </div>
      )}
